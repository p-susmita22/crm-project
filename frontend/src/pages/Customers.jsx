...
import { FiPlus, FiEdit2, FiTrash2, FiUsers, FiSearch, FiFilter, FiDownload, FiFileText, FiEye, FiXCircle, FiCalendar, FiRefreshCw, FiSend, FiInbox, FiClock } from 'react-icons/fi';
  ...
  const handleExportPDF = () => {
    ...
    const rows = filteredCustomers.map(c => `
      <tr>
        ...
        <td>${c.fullAddress || '-'}</td>
        <td class="status-${
          c.status === 'Interested' || c.status === 'Onboarded' ? 'interested' :
          c.status === 'Rejected' ? 'rejected' :
          c.status === 'Follow up' || c.status === 'Document pending' ? 'others' : 'pending'
        }">${c.status || 'Pending'}</td>
        <td>${c.notes || '-'}</td>
      </tr>
    `).join('');
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  {formData.state === 'Odisha' && (
                    <datalist id="odisha-districts-customer">
      ...
      {showSubmissionsModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
          <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl border border-gray-100 dark:border-gray-700">
            <div className="p-6 border-b border-gray-100 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 bg-gray-50 dark:bg-gray-800/50">
              <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
